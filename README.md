...
- **Google Sign-In** uses Credential Manager + Sign in with Google on Android, and the GoogleSignIn-iOS SDK on iOS. Backend verifies the ID token with `google-auth-library` against `GOOGLE_WEB_CLIENT_ID` (the Web OAuth Client ID from Google Cloud Console), then mints a Supabase session via `signInWithIdToken`. Android passes the same Client ID to Credential Manager as `serverClientId` (built into the APK via `-PgoogleWebClientId=...` or the `GOOGLE_WEB_CLIENT_ID` env var at build time). iOS needs a separate iOS OAuth Client ID for the bundle id `com.mitimaiti.MitiMaiti` — fill it into `ios/MitiMaiti/Config/GoogleSignInConfig.swift` along with the same web Client ID, add the GoogleSignIn-iOS package via Xcode (File → Add Package Dependencies → `https://github.com/google/GoogleSignIn-iOS`, add `GoogleSignIn` and `GoogleSignInSwift` products to the MitiMaiti target), and add a URL Type whose scheme is the reversed iOS client ID (Target → Info → URL Types). Supabase Auth dashboard → Providers → Google must also be enabled and configured with the same Web Client ID.
- **Sign in with Apple** runs natively on iOS via `AuthenticationServices`. The `Sign in with Apple` capability needs to be enabled on the App ID `com.mitimaiti.MitiMaiti` in the Apple Developer portal (Identifiers → App IDs) and on the Apple provider in the Supabase Auth dashboard (Service ID + key) before device/production builds; simulator builds work as long as a real iCloud account is signed in.
